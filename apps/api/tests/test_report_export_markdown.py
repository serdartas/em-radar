    ...
    with session_factory() as session:
        ...
        report = ReportTable(
            ...
            findings_count_by_severity={
                Severity.INFO: 0,
                Severity.WARNING: 0,
                Severity.CRITICAL: 1 if include_finding else 0,
            },
        )
        session.add(report)
        session.commit()
...
def test_export_markdown_returns_markdown_body(
    ...
    # A succeeded report carries no failure notice.
...
def test_export_markdown_failed_report_shows_failure_notice(
    api_client: TestClient, session_factory: sessionmaker[Session]
) -> None:
    report_id = _persist_report(
        session_factory,
        status=ReportStatus.FAILED,
        error="all data sources failed: board data unavailable: ConnectorAuthError",
    ...
    assert response.status_code == 200
    body = response.text
    # A failed run must be unmistakable rather than looking like a zero-finding report.
    assert "> **This report did not complete successfully.**" in body
    assert "> **Status:** Failed" in body
    assert "- **Status:** Failed" in body
    assert (
        "> **Error:** all data sources failed: board data unavailable: ConnectorAuthError" in body
    )
